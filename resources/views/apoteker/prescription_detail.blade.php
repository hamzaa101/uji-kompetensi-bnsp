...
<div class="space-y-4">
    <x-page-header
        ...
        :description="'Pasien: '.($order->user?->name ?? '-').' | Status: '.$order->status"
    />

    <div class="grid gap-4 xl:grid-cols-[1fr_360px]">
    <div class="panel">
        <h2 class="section-title">Item Order</h2>
        <div class="table-wrap mt-4">
            <table><thead><tr><th>Obat</th><th>Qty</th><th>Harga</th><th>Subtotal</th></tr></thead><tbody>
                @foreach($order->items as $item)
                    <tr><td class="font-medium">{{ $item->medicine->name }}</td><td>{{ $item->quantity }}</td><td>Rp {{ number_format($item->price, 0, ',', '.') }}</td><td>Rp {{ number_format($item->subtotal, 0, ',', '.') }}</td></tr>
                @endforeach
            </tbody></table>
        </div>
    ...
    <div class="panel">
        ...
        <img class="preview-image mt-4 h-80 w-full" src="{{ asset('storage/'.$order->prescription->image_path) }}" alt="Resep dokter">
        @if($order->prescription->status === 'pending')
            ...
            <form class="mt-3 space-y-3" method="post" action="{{ route('apoteker.prescriptions.reject', $order) }}">
                @csrf
                <label class="field"><span>Alasan Reject</span><textarea name="notes" rows="3">{{ old('notes') }}</textarea><x-field-error name="notes" /></label>
