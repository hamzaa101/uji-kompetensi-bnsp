            ...
            <table><thead><tr><th>Waktu</th><th>User</th><th>Action</th><th>Tabel</th><th>IP</th><th>User Agent</th><th>Deskripsi</th></tr></thead><tbody>
                @forelse($logs as $log)
                    <tr>
                        ...
                        <td><span class="status status-info">{{ $log->action }}</span></td>
                        <td>{{ $log->table_name ? $log->table_name.' #'.$log->record_id : '-' }}</td>
                        <td>{{ $log->ip_address ?: '-' }}</td>
                        <td class="text-cell">{{ $log->user_agent ?: '-' }}</td>
                        <td class="text-cell">{{ $log->description ?: '-' }}</td>
                    </tr>
                @empty
